...
#         sf_output = {"siegfried": log["matches"][0]}
#     else:
#         sf_output = {"siegfried": "unknown"}
#         logging.warning(
#             "the file {} has no siegfried info".format(log["filename"]))
#     del log["filename"]
#     if sf_output["siegfried"]:
#         sf_output["siegfried"] = replace_none_values(sf_output["siegfried"])
#     return sf_output

# That oddly seems to work
...
def add_sf_info_to_db(sf_log_path, session, File):
    with open(sf_log_path, "r") as sf_log:
        ...
        session.commit()
        # Really curious if this runs or if we get the exception that ["matches"] is empty ... cause it shouldn't go into that exception ...